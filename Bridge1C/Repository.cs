namespace Bridge1C
{
    ...
    public class Repository : IRepository
    {
                ...
                switch (propertyName)
                {
                    ...
                    case Requisites.ExCode_Ware:
                        ...
                        запрос.Текст = @"   ВЫБРАТЬ
                                                СопоставлениеНоменклатуры.Номенклатура, СопоставлениеНоменклатуры.Контрагент
                                            ИЗ
                                                РегистрСведений.СопоставлениеНоменклатуры как СопоставлениеНоменклатуры
                                            ГДЕ
                                                СопоставлениеНоменклатуры.Контрагент.Код = &КодКонтрагента
                                            И
                                                СопоставлениеНоменклатуры.ВнешнийКод = &ВнешнийКод";
        ...
        public List<dynamic> GetWareExCodes(dynamic ware)
        {
            ...
            var выборка = this.Connector.Connection.РегистрыСведений.СопоставлениеНоменклатуры.Выбрать(отборТовар);
        ...
        public bool UpdateWareExCode(string innerCode, string exCode, string supplierCode)
        {
            ...
            try
            {
                var менеджерЗаписи = this.Connector.Connection.РегистрыСведений.СопоставлениеНоменклатуры.СоздатьМенеджерЗаписи();
        ...
        public bool AddNewExCode(dynamic ware, dynamic counteragent, string value)
        {
            try
            {
                ...
                var менеджер = this.Connector.Connection.РегистрыСведений.СопоставлениеНоменклатуры.СоздатьМенеджерЗаписи();
